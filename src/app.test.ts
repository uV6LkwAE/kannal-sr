...
describe("health endpoints", () => {
  ...
  it("prevents caching of the public health response", async () => {
    const response = await app.request("/200")
...
describe("contact API", () => {
  ...
  it.each([
    ...
    const response = await postContact(createApp(createDependencies()), validContact, contentType)
    expect(response.status).toBe(status)
    expect((await response.json()).error.code).toBe(code)
  })

  it("rejects malformed JSON", async () => {
    const response = await postContact(createApp(createDependencies()), "{")
  ...
  it.each([
    ...
    const dependencies = createDependencies()
    const response = await postContact(createApp(dependencies), body)
  ...
  it.each([
    ...
    const response = await postContact(createApp(dependencies), validContact)
...
describe("page rendering", () => {
  ...
  it("renders the home page through the shared layout", async () => {
    ...
    expect(body).toContain('"@type":["ProfessionalService","LocalBusiness"]')
  })
